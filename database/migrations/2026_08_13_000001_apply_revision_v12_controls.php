...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('gate_passes', function (Blueprint $table) {
            $table->foreignId('prepared_verified_by_user_id')
                ->nullable()
                ->constrained('users')
                ->nullOnDelete();

            $table->foreignId('prepared_verifier_signature_snapshot_id')
                ->nullable()
            ...
            $table->foreignId('approver_signature_snapshot_id')
                ->nullable()
                ->constrained('signature_snapshots')
                ->nullOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::table('custody_transactions', function (Blueprint $table) {
            $table->dropForeign('ct_laundry_borrower_sig_fk');
            $table->dropColumn('laundry_borrower_signature_snapshot_id');

            $table->dropConstrainedForeignId('laundry_approved_by_user_id');

            $table->dropForeign('ct_laundry_approver_sig_fk');
            $table->dropColumn('laundry_approver_signature_snapshot_id');
        ...
        Schema::table('request_items', function (Blueprint $table) {
            $table->dropColumn('use_location');
        });

        Schema::table('request_versions', function (Blueprint $table) {
            $table->dropColumn('represents_student_activity');
        });

        Schema::dropIfExists('temporary_delegations');

        Schema::table('users', function (Blueprint $table) {
            $table->dropColumn('access_classification');
        });
    }
};