...
class ConditionalProcessingController extends Controller
{
    public function signGatePassVerified(Request $request, GatePass $gatePass, SignatureService $signatures, DocumentService $documents, AuditService $audit): RedirectResponse
    {
        ...
        return back()->with('status', 'Gate Pass Verified By signature recorded. SPMU Head approval is required before printing.');
    }
    ...
    public function signGatePassApproved(Request $request, GatePass $gatePass, SignatureService $signatures, DocumentService $documents, AuditService $audit): RedirectResponse
    {
        ...
        if (! $gatePass->prepared_verified_at || $gatePass->prepared_verified_by_user_id === $request->user()->id || $gatePass->custody->borrower_user_id === $request->user()->id) {
        ...
        return back()->with('status', 'Gate Pass digitally approved and ready for printing. The guard signs after campus exit.');
    }
    ...
    public function approveLaundryForm(Request $request, CustodyTransaction $custody, SignatureService $signatures, DocumentService $documents, AuditService $audit): RedirectResponse
    {
        ...
        if (! $custody->laundry_borrower_signature_snapshot_id || $custody->borrower_user_id === $request->user()->id) {
        ...
        return back()->with('status', 'Laundry Form contains the Borrower and SPMU Head digital signatures and is ready for printing.');
    }
    ...
    public function gatePass(Request $request, GatePass $gatePass, AuditService $audit): RedirectResponse
    {
        ...
        return back()->with('status', 'Gate Pass paper signature and physical event verified.');
    }
    ...
    public function laundry(Request $request, LaundryRecord $laundry, AuditService $audit, DocumentService $documents): RedirectResponse
    {
        ...
        DB::transaction(function () use ($laundry, $request, $data, $documentId, $audit, $documents): void {
            ...
            if (! $openLaundry && ! $openIncident && ! $openOverdue && $allReturned) {
        ...
        return back()->with('status', 'Laundry evidence and physical condition verified. Clean linen returned to Available.');
    }
}
