...
class BorrowingRequestController extends Controller
{
        ...
        if ($workspace === 'BORROWER') {
            ...
            $query->where(function ($query) use ($request, $workspace): void {
                $query->where('status', 'UNDER_'.$workspace)
                    ->orWhereHas('currentVersion.approvalSteps', fn ($step) => $step->where('stage_code', $workspace)->where('approver_user_id', $request->user()->id));
            });
        }
    ...
    public function create(): View
    {
        return view('requests.form', [
            ...
            'items' => InventoryItem::with('unit')->where('active', true)->where('borrowable', true)->orderBy('unique_description')->get(),
        ]);
    }

    public function store(Request $request, InventoryService $inventory, DocumentService $documents): RedirectResponse
    {
        $data = $this->validateRequest($request);
        $user = $request->user();
        if ($user->activeRestrictions()->exists()) {
            throw ValidationException::withMessages(['request' => 'An active borrowing restriction prevents a new request.']);
        }

        $borrowingRequest = DB::transaction(function () use ($data, $user, $inventory): BorrowingRequest {
            ...
            $version = $borrowingRequest->versions()->create($this->versionData($data, $user->id, 1));
        ...
        return redirect()->route('requests.show', $borrowingRequest)->with('status', 'Draft request and official preview generated. Review the letter before certifying and submitting it.');
    }
    ...
    {
        abort_unless($borrowingRequest->borrower_user_id === $request->user()->id && in_array($borrowingRequest->status, [RequestStatus::Draft, RequestStatus::ReturnedForRevision], true), 403);
    ...
    public function update(Request $request, BorrowingRequest $borrowingRequest, InventoryService $inventory, DocumentService $documents): RedirectResponse
    {
        abort_unless($borrowingRequest->borrower_user_id === $request->user()->id && in_array($borrowingRequest->status, [RequestStatus::Draft, RequestStatus::ReturnedForRevision], true), 403);
        $data = $this->validateRequest($request);
        DB::transaction(function () use ($borrowingRequest, $data, $request, $inventory): void {
            $versionNo = $borrowingRequest->status === RequestStatus::ReturnedForRevision ? $borrowingRequest->current_version_no + 1 : $borrowingRequest->current_version_no;
            if ($borrowingRequest->status === RequestStatus::ReturnedForRevision) {
                $version = $borrowingRequest->versions()->create($this->versionData($data, $request->user()->id, $versionNo));
                $borrowingRequest->update(['current_version_no' => $versionNo, 'status' => RequestStatus::Draft]);
            } else {
                $version = $borrowingRequest->currentVersion;
                $version->update($this->versionData($data, $request->user()->id, $versionNo));
                $version->items()->delete();
            }
            $this->saveItems($version, $data, $inventory);
        });

        $borrowingRequest->fresh()->currentVersion->documents()->where('document_type', 'REQUEST_LETTER')->where('status', 'DRAFT')->update(['status' => 'SUPERSEDED', 'invalidated_at' => now(), 'invalidation_reason' => 'Draft was regenerated after borrower editing.']);
        $documents->requestLetter($borrowingRequest->fresh(), false);

        return redirect()->route('requests.show', $borrowingRequest)->with('status', 'Request draft updated.');
    }

    public function submit(Request $request, BorrowingRequest $borrowingRequest, RequestWorkflowService $workflow): RedirectResponse
    {
    ...
    public function cancel(Request $request, BorrowingRequest $borrowingRequest, RequestWorkflowService $workflow): RedirectResponse
    {
        ...
        $workflow->cancel($borrowingRequest, $request->user(), $data['reason']);

        return back()->with('status', 'Request cancelled and any unreleased allocation restored.');
    }
    ...
    private function validateRequest(Request $request): array
    {
        return $request->validate([
            ...
            'student_organization' => ['nullable', 'string', 'max:255'],
            'represents_student_activity' => ['nullable', 'boolean'],
            'represented_program_department' => ['required_if:represents_student_activity,1', 'nullable', 'string', 'max:255'],
            'represented_year_level' => ['required_if:represents_student_activity,1', 'nullable', 'string', 'max:40'],
            'event_details' => ['required', 'string', 'max:2000'],
            'remarks' => ['nullable', 'string', 'max:1000'],
            'off_campus' => ['nullable', 'boolean'],
            'item_ids' => ['required', 'array', 'min:1'],
            'item_ids.*' => ['required', 'integer', 'exists:inventory_items,id'],
            'quantities' => ['required', 'array'],
            'quantities.*' => ['nullable', 'numeric', 'min:0'],
            'locations' => ['nullable', 'array'],
            'locations.*' => ['nullable', Rule::in(['ON_CAMPUS', 'OFF_CAMPUS'])],
        ]);
    }

    private function versionData(array $data, int $userId, int $versionNo): array
    {
        return [
            ...
            'represents_student_activity' => (bool) ($data['represents_student_activity'] ?? false),
            'student_organization' => $data['student_organization'] ?? null,
            'represented_program_department' => $data['represented_program_department'] ?? null,
            'represented_year_level' => $data['represented_year_level'] ?? null,
            'event_details' => $data['event_details'],
            'off_campus' => collect($data['locations'] ?? [])->contains('OFF_CAMPUS'),
            'remarks' => $data['remarks'] ?? null,
            'created_by_user_id' => $userId,
        ];
    }

    private function saveItems(RequestVersion $version, array $data, InventoryService $inventory): void
    {
        $selected = 0;
        foreach ($data['item_ids'] as $itemId) {
            $quantity = (float) ($data['quantities'][$itemId] ?? 0);
            if ($quantity <= 0) {
                continue;
            }
            $item = InventoryItem::with('unit')->where('active', true)->where('borrowable', true)->findOrFail($itemId);
            $balance = $inventory->availability($item, Carbon::parse($data['needed_from']), Carbon::parse($data['return_due_at']));
            if ($quantity > $balance['available']) {
                throw ValidationException::withMessages(['quantities' => "{$item->unique_description} has only {$balance['available']} available for the complete period."]);
            }
            $location = strtoupper((string) ($data['locations'][$item->id] ?? 'ON_CAMPUS'));
            if (! $item->off_campus_allowed && $location !== 'ON_CAMPUS') {
                throw ValidationException::withMessages(['locations' => "{$item->unique_description} is restricted to On-Campus use."]);
            }
            if (! in_array($location, ['ON_CAMPUS', 'OFF_CAMPUS'], true)) {
                throw ValidationException::withMessages(['locations' => 'Choose a valid campus location for each selected item.']);
            }
        ...
        if ($selected === 0) {
            throw ValidationException::withMessages(['items' => 'Enter a quantity greater than zero for at least one item.']);
        }
    }
